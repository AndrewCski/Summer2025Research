function [init_r, init_v_vectors, N_total, particle_traits] = Plume_Dust_Starting_States(plume_theta, ...
    plume_phi, theta_p, d_theta, direction_angles, init_speeds, q_over_m, radii, densities)

R_E = 1560e3;                                         % Europa radius (m)

plume_phi = mod(plume_phi * pi / 180, 2*pi) - pi;
plume_theta = plume_theta * pi / 180;
plume_r = [(R_E+1) * cos(plume_theta) * cos(plume_phi), ...
    (R_E+1) * cos(plume_theta) * sin(plume_phi), (R_E+1) * sin(plume_theta)];
                ...
                for d = 1:length(q_over_m)
                    for i = 1:length(radii)
                        for j = 1:length(densities)
                            ...
                            particle_traits(index, 5) = 4/3 * pi * radii(i)^3 * densities(j);

...
[X, Y] = meshgrid(x_range, z_range);                               % #1 !!!
Z = x_plane * ones(size(X));                                       % #2 !!!
...
figure;
imagesc(x_range/1e3, y_range/1e3, B_z * 1e9);                      % #5 !!!
set(gca,'YDir','normal'); % I think this is supposed to be Ydir???
colorbar;
xlabel('x (km)')                                                   % #6 !!!
ylabel('y (km)')                                                   % #7 !!!
title(sprintf('Europa Magnetic Environment at time step %d/%d', time_idx, numsteps));

hold on
streamslice(X/1e3, Y/1e3, B_x, B_y);                               % #8 !!!
hold off
...
[X, Y] = meshgrid(x_range, z_range);
Z = x_plane * ones(size(X));
...
figure;
imagesc(x_range/1e3, y_range/1e3, B_z * 1e9);
set(gca,'YDir','normal'); % I think this is supposed to be Ydir???
colorbar;
xlabel('x (km)')
ylabel('y (km)')
title(sprintf('B_z (nT) at time step %d/%d', time_idx, numsteps));

hold on
streamslice(X/1e3, Y/1e3, B_x, B_y);  % Automatically plots streamlines
hold off
...
% xlabel('x (km)')
% ylabel('y (km)')
% title('Magnetic Field Streamlines');

% % Optional: add arrows for better appearance
% hold on;
% quiver(X/1e3, Y/1e3, B_x, B_y, 'k');
% hold off
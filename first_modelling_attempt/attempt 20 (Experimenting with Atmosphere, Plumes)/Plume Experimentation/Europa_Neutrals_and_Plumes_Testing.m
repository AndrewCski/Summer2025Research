...
% polar angles
thetas = acos(dotRV ./ normV);   % θ in radians
plume_points = thetas < pi/2;

for i = 1:numel(N_n)
    if plume_points(i)
        N_n(i) = N_n(i) + Np0 * exp((-norm(plume_r - [X(i), Y(i), Z(i)]) + R_E)/Hp) * exp(-(thetas(i)/thetap)^2);
    end
end
...
set(gca,'YDir','normal');
cb = colorbar; cb.Label.String = 'parts/cm^-3'; set(gca, 'ColorScale', 'log'); clim([1e3 5e13]);

function [ ] = Europa_Particle_Flux(r_cell, first_step, last_step, dims, resolution, ...
...
for i = 1:length(r_cell)
    ...
    visited_bins = false(resolution, resolution);
    ...
    for j = first_step:last_step
        ...
        if ~visited_bins(idx2, idx1)  % rows = y, cols = x
            Flux_Counts(idx2, idx1) = Flux_Counts(idx2, idx1) + 1;
            visited_bins(idx2, idx1) = true;
        end
    end
end
...
ylabel([dims(2), ' (R_E)']);
if energy < 1e6
    title(sprintf('Particle Flux Counts for: %s %.1f KeV, from t = %.2f to %.2f hours', ...
        element, energy / 1000, (start_time + first_step * abs(time_step))/3600, ...
        (start_time + last_step * abs(time_step))/3600));
else
    title(sprintf('Particle Flux Counts for: %s %.1f MeV, from t = %.2f to %.2f hours', ...

...
period = 2 * pi / (200 * omega);         % modelling 1/200 of a synodic period
time = linspace(0, period, numsteps);
timestep = period / numsteps;

% B_prim points mainly along x and y (x: dir of corotation, y: toward jupiter)
B0_vec = [75 * 10 ^-9; 200 * 10 ^-9; 0];  % 75 nT azimuthal, 200 nT radial 
% (-350 nT constant added to North-South, represented by z, in loop)
...
r(:,1) = [R_E * 2; 0; 0];  % initial position
v(:,1) = [950000; 0; 0];   % initial velocity

for n = 1:numsteps-1

    % adding constant z-dir B field at end of B expression here
    B = B_sec(r(:,n), M_real(:,n)) + B_prim(:,n) + [0; 0; -350 * 10^-9]; 
...
energies = .5 .* m .* vecnorm(v).^2 * 6.242 * 10 ^ 18;

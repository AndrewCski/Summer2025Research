    ...
    % surface of Europa at closest approach, theta is the longitude of 
    % the base of the plume at Europa's surface, and start_factor
    % determines the point in Europa's synodic period.

R_E = 1560e3;                                % Europa radius (m)
omega = 2*pi/(11.23*3600);                   % synodic frequency (rad/s)
start_factor = 0;
period_time = start_factor * (2 * pi / omega)/4;
...
filename = sprintf('SpaceCraft Measurements t%d Theta = %d y = %.2f R_E.gif', start_factor * 25, ...
    plume_theta, dist./R_E);
